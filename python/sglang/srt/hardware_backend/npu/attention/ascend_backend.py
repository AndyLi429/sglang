...
from sglang.srt.layers.attention.cp_utils import cp_split_tensor_by_zigzag
...
class AscendAttnBackend(AttentionBackend):
    ...
    def _all_gather_kv_for_cp(
        ...
        if (
            ...
            or not enable_prefill_cp(forward_batch, self.enable_prefill_cp)
            or self.pcp_size <= 1
        ):
            return tensor
    ...
    def forward_extend(
        ...
        if not self.use_mla:
            ...
            if self.use_fia or layer.qk_head_dim == 256:
                ...
                if enable_prefill_cp(forward_batch, self.enable_prefill_cp):
                    metadata = forward_batch.gqa_cp_metadata
                    q = cp_split_tensor_by_zigzag(
                        q, metadata.split_list, metadata.zigzag_index
                    )
                    num_token_padding = q.shape[0]
                    k = self._all_gather_kv_for_cp(k, forward_batch)
                    v = self._all_gather_kv_for_cp(v, forward_batch)
                    local_seq_len = metadata.per_rank_actual_token[
                        get_pcp_group().rank_in_group
                    ]
                    actual_seq_lengths_q = np.cumsum([local_seq_len])
                    actual_seq_lengths_kv = np.cumsum([metadata.total_seq_lens])
        ...
        elif enable_prefill_cp(forward_batch,self.enable_prefill_cp):
            q_nope, q_rope = q.split([layer.v_head_dim, self.qk_rope_head_dim], dim=-1)
            k_nope, k_rope = k.split([layer.v_head_dim, self.qk_rope_head_dim], dim=-1)
