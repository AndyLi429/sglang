...
class MQALayer(MqaAttentionBase):
    ...
    def _forward_prepare_multi_stream_npu(
        ...
        # Compressor only depends on x, so launch it before the Q/KV branches.
        ...
        # Cube phase: launch the two projections independently.
        ...
        # Vector phase: norm/RoPE/cache write can overlap the other branches.
        with torch.npu.stream(stream_kv):
            kv = self.kv_norm(kv)
        ...
        # Indexer runs on the current stream in parallel with Q/KV/compressor.
        if self.indexer is not None:
            self.indexer(
                x=x,
                q_lora=q_lora,
        ...
        # Join all producers before downstream attention consumes their output.
        current_stream.wait_stream(stream_kv)
